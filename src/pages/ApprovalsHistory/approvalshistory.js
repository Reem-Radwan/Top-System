...
// import { MOCK, COLS, MONTH_NAMES, COMPANIES } from "../../data/approvalshistorydata";
...
// function getVisibleCols(meta) {
//   const showBase = !!(meta && meta.show_base_price);
//   return COLS.filter((c) => !c.baseOnly || showBase);
// }

// // ─────────────────────────────────────────────
// //  COLUMN FILTER MENU (rendered as a portal)
// // ─────────────────────────────────────────────
...
//   // ── Date tree state (only computed when col.type === "date") ──
...
//   const setYearChecked = (yr, checked) => {
...
//               const allMonthKeys = months.map(
//                 (m) => `${yr}-${String(m + 1).padStart(2, "0")}`
//               );
//               const allSelected =
//                 allMonthKeys.length > 0 && allMonthKeys.every((k) => checkedMonths[k]);
//               return (
...
//                       onChange={(e) => {
//                         e.stopPropagation();
//                         setYearChecked(yr, e.target.checked);
//                         // apply after state update
//                         const next = { ...checkedMonths };
//                         months.forEach((m) => {
//                           const key = `${yr}-${String(m + 1).padStart(2, "0")}`;
...
//                             <input
//                               type="checkbox"
//                               checked={!!checkedMonths[key]}
//                               onChange={(e) => handleMonthChange(key, yr, e.target.checked)}
//                             />
...
//         <input
//           type="text"
//           className="ah__filter-menu-search"
//           placeholder="Search..."
//           value={textSearch}
...
//                 className="ah__fm-cb"
//                 value={String(val)}
//                 checked={!!checkedText[String(val)]}
//                 onChange={(e) => handleTextCbChange(String(val), e.target.checked)}
//               />
//               {String(val)}
//             </label>
//           ))}
...
// //  TABLE HEADER
// // ─────────────────────────────────────────────
// function TableHeader({ meta, sortState, activeColumnFilters, onSort, onOpenFilter }) {
//   const cols = getVisibleCols(meta);
...
//                 </span>
//                 <span
//                   className={`ah__sort-indicator${isActive ? " ah__active" : ""}`}
//                   onClick={() => onSort(col.key)}
//                 >
//                   {arrow}
//                 </span>
//                 <button
//                   className={`ah__filter-btn-th${hasColFilter ? " ah__active" : ""}`}
//                   onClick={(e) => onOpenFilter(e, col.key)}
...
//               <div className="ah__salesman-avatar">
...
//           <td>
//             <span className="ah__badge-unit">{r.unit_code || "-"}</span>
//           </td>
...
//                 {r.land_area != null
//                   ? <>{fmtNumber(r.land_area)}&nbsp;<small className="ah__area-unit">m²</small></>
//                   : "-"}
//               </td>
//               <td className="ah__text-muted-sm" style={{ whiteSpace: "nowrap" }}>
//                 {r.gross_area != null
//                   ? <>{fmtNumber(r.gross_area)}&nbsp;<small className="ah__area-unit">m²</small></>
//                   : "-"}
//               </td>
...
//   // Filter menu
//   const [filterMenu, setFilterMenu] = useState(null); // { field, position }

//   // ── Sort logic ──
//   const toggleSort = useCallback((col) => {
//     setSortState((prev) => {
//       if (prev.column === col) {
//         return { column: col, direction: prev.direction === 1 ? -1 : 1 };
//       }
...
//   // ── Recompute filtered whenever deps change ──
//   useEffect(() => {
//     if (!raw.length && !selectedCompany) return;
...
//   }, [raw, search, dateFrom, dateTo, minSales, maxSales, activeColumnFilters, sortState, applyFilters, selectedCompany]);

//   // ── Debounced search/sales ──
//   const debouncedSetSearch = useCallback(debounce((v) => setSearch(v), 200), []);
//   const debouncedSetMin = useCallback(debounce((v) => setMinSales(v), 200), []);
//   const debouncedSetMax = useCallback(debounce((v) => setMaxSales(v), 200), []);

//   // ── Load company data ──
//   const loadData = useCallback((companyKey) => {
...
//       const sortedRaw = [...dataset.rows].sort((a, b) => (b.date_ts || 0) - (a.date_ts || 0));
//       setMeta(dataset.meta);
//       setRaw(sortedRaw);
//       setMetaLine(`✅ Approved requests only • ${dataset.meta.company_name} • Newest to oldest`);
...
//     e.stopPropagation();
//     if (filterMenu && filterMenu.field === field) {
//       setFilterMenu(null);
//       return;
//     }
...
//               <h2>Filters</h2>
//               <button className="ah__reset-btn" onClick={resetFilters}>
//                 ↺ Reset All Filters
//               </button>
//             </div>
...
//                 <input
//                   className="ah__filter-input"
//                   placeholder="Salesman, client, or unit..."
//                   defaultValue={search}
//                   onChange={(e) => debouncedSetSearch(e.target.value)}
//                   key={`search-${selectedCompany}`}
//                 />
//               </div>
//               <div className="ah__filter-group">
...
//               Showing{" "}
//               <span className="ah__shown">{filtered.length}</span>
...
//                   </tbody>
//                 ) : isDashboardVisible && meta ? (
//                   <TableBody rows={filtered} meta={meta} />
//                 ) : (
//                   <tbody>
//                     <tr>
//                       <td colSpan={8}>
...
// ✅ FIX 1 & 4: debounce defined outside component with a proper shared timer per instance
function debounce(fn, ms = 250) {
  let t = null;
  return (...args) => {
    clearTimeout(t);
...
export default function ApprovalsHistory() {
  ...
  // ✅ FIX 2: Create stable debounced setters with useMemo so debounce timers
  // are shared correctly across renders and never recreated unnecessarily.
  ...
  }, []); // ✅ FIX 5: no external deps — pure function of its arguments
  ...
  // ✅ FIX 3: Removed `selectedCompany` from deps — it's not used inside the effect.
  // The effect already re-runs when `raw` changes (which resets on company change).
  useEffect(() => {
    const result = applyFilters(raw, search, dateFrom, dateTo, minSales, maxSales, activeColumnFilters, sortState);
    setFiltered(result);
    setStatusText(`Showing ${result.length} result(s).`);
