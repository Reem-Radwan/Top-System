...
import { mockUnits, mockCompanies } from "../../data/manageinventorydata";
...
/* ── Columns icon — FontAwesome 6 "table-columns" (fa-solid fa-table-columns) ── */
...
export default function ManageInventory() {
  ...
  const allDataRef        = useRef(mockUnits.map((u) => ({ ...u })));
  ...
  // ── Unit Code protection — SweetAlert popup (same style as bulk-update) ───────
  ...
  const applyAllFilters = useCallback((company, afOverride, rfOverride) => {
    const comp = company    ?? selectedCompany;
    const af   = afOverride ?? activeFilters;
    const rf   = rfOverride ?? rangeFilters;
    if (!comp) { setFilteredData([]); filteredDataRef.current = []; return; }
    const result = allDataRef.current.filter((row) => {
      if (row.company !== comp) return false;
      for (const key in af) {
  ...
  }, [selectedCompany, activeFilters, rangeFilters]);
  ...
  // unit_code is visually editable but saving is blocked — see unit code cell JSX
  ...
  const saveCell = useCallback((actualIdx, field, rawNewVal) => {
    // Guard: unit_code must never be saved
  ...
  const handleMultiSelectDown = useCallback((rowIdx, field) => {
    // Block multi-select on unit_code
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === "ArrowDown") {
        ...
        if (td) {
          // Block multi-select on unit_code via keyboard shortcut too
      ...
      if (e.key === "Delete") {
        e.preventDefault();
        // If focused on a unit_code cell, show protection message and bail
        const td = document.activeElement?.closest("td[data-row]");
                              ...
                              return (
                                <td key={col.field} className={tdClass}
                                  ...
                                  {col.type === "select" ? (
                                    ...
                                    /* ── Unit Code: fully editable, but saving is blocked.
                                       On blur — if the user changed the text — show a SweetAlert
                                       warning and reset the cell content back to the original value. */
                                    <div
                                      ...
                                      onFocus={() => {
                                        preEditRef.current = dispVal;
                                        clearMultiSelectHighlight();
                                      }}
                                      onBlur={(e) => {
                                        const typed = e.currentTarget.innerText.trim();
                                        if (typed !== preEditRef.current) {
                                          // Revert the DOM content immediately
                                          e.currentTarget.innerText = preEditRef.current ?? "";
                                          // Fire the SweetAlert warning
                                          if (window.Swal) {
                                            window.Swal.fire({
                                              icon: "error",
                                              title: "Error",
                                              text: "Unit Code cannot be changed.",
                                              confirmButtonColor: "#6c757d",
                                              confirmButtonText: "OK",
                                            });
                                          } else {
                                            fallbackToast("info", "Unit Code cannot be changed.", 2500);
                                          }
                                        }
                                      }}
                                      onKeyDown={(e) => {
                                  ...
                                  {/* Never show drag handle on unit_code column */}
