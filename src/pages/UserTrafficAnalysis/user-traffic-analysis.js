...
import { MOCK, UTA_PRO, UTA_EXCL, utaCatOf } from '../../data/user-traffic-analysisdata';
...
function buildCohort(rows) {
  const ufm = {};
  rows.forEach(r => { const m = r.dt.getFullYear()*100+r.dt.getMonth(); if (!ufm[r.uid]||m<ufm[r.uid]) ufm[r.uid]=m; });
