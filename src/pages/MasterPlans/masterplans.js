...
// import { mockProjects, getMasterplanData, getUnitDetails } from '../../data/masterplansdata';
...
//         <i className="fa-solid fa-map" style={{color:'#007bff',marginRight:8,fontSize:14}}/>
...
//                 {String(p.id)===String(value)&&<i className="fa-solid fa-check" style={{marginLeft:'auto',color:'#007bff'}}/>}
//               </div>
//             ))}
//           </div>
//         </div>
...
//                 {isSel&&<i className="fa-solid fa-check" style={{marginRight:6,fontSize:10,color:'#007bff'}}/>}{opt}
//               </div>
//             );})}
//         </div>, document.body
//       )}
...
// /* ── Layout Modal — CHANGE 3: catalog-style design ── */
...
// /* ── Single Tooltip ── */
// function SingleTooltip({ unit, companyId, markerRect, onClose, onBuy, onViewLayout, onShowInCatalog }) {
//   const hasBrochure = unit.layout_images?.length > 0;
//   return (
...
//         {hasBrochure&&<button className="buy-now-btn" style={{backgroundColor:'#17a2b8',marginTop:5,marginBottom:5}} onClick={()=>onViewLayout(unit.layout_images)}><i className="fa-regular fa-images"/> View Brochure</button>}
//         {(unit.status==='Available'||IS_MANAGER)&&<button className="buy-now-btn" onClick={()=>onBuy(unit.unit_code)}>Buy Now</button>}
//         <div style={{textAlign:'center',marginTop:8,paddingTop:8,borderTop:'1px solid #eee'}}>
//           <button style={{fontSize:12,color:'#0d6efd',background:'none',border:'none',cursor:'pointer',fontWeight:'bold',padding:0}} onClick={()=>onShowInCatalog(unit.unit_code,companyId)}>
...
// /* ── Building Tooltip ── */
// function BuildingTooltip({ buildingName, units, companyId, markerRect, onClose, onBuy, onViewLayout, urlFilters, frontendFilters, onShowInCatalog }) {
...
//     if(!IS_MANAGER&&u.status!=='Available')return false;
//     return true;
//   });
//   return (
...
//                     {u.layout_images?.length>0&&<button className="btn btn-sm btn-info text-white" style={{padding:'2px 6px',fontSize:11,border:'none',borderRadius:3}} onClick={()=>onViewLayout(u.layout_images)}><i className="fa-regular fa-images"/></button>}
//                     <button className="btn btn-sm btn-light border" style={{padding:'2px 6px',fontSize:11,color:'#0d6efd'}} onClick={()=>onShowInCatalog(u.unit_code,companyId)}><i className="fa-solid fa-circle-info"/></button>
//                     {(u.status==='Available'||IS_MANAGER)&&<button className="buy-btn" style={{margin:0,padding:'2px 8px',fontSize:11}} onClick={()=>onBuy(u.unit_code)}>Buy</button>}
//                   </div></td>
//                 </tr>
//               ))}</tbody>
//             </table>
...
// /* ── CHANGE 1 & 2: Filter Modal — gray header, MultiSelect for Bedrooms & Status ── */
...
//         {/* ── CHANGE 1: Gray header ── */}
//         <div className="fmr-head fmr-head-gray">
...
//           {/* Row: Bedrooms + Finishing */}
//           <div className="fmr-row2">
//             {/* CHANGE 2a: Bedrooms → MultiSelect dropdown */}
...
//               <MultiSelect
//                 options={allOpts.beds}
//                 selected={filterSel.beds}
//                 onChange={v=>set('beds',v)}
//                 placeholder="Select bedrooms…"
//                 validSet={validSets.beds}
//               />
...
//             </div>
//             <div style={{fontSize:11,color:'#9ca3af',marginTop:4}}>Available: {dynArea.min}–{dynArea.max} m²</div>
//           </div>

//           {/* CHANGE 2b: Status → MultiSelect dropdown */}
//           {!isClient&&(
//             <div className="fmr-grp">
...
//               <MultiSelect
//                 options={allOpts.stat}
//                 selected={filterSel.stat}
//                 onChange={v=>set('stat',v)}
//                 placeholder="Select status…"
//                 validSet={validSets.stat}
//               />
//               {allOpts.stat.length===0&&<span className="fmr-empty-txt">—</span>}
//             </div>
//           )}
//         </div>
...
// function Legend() {
//   const items=[{color:'#28a745',label:'Available'},{color:'#dc3545',label:'Unavailable/Sold'},{color:'#ffc107',label:'Blocked'},{color:'#045eef',label:'Building'},{color:'#6c757d',label:'Other'}];
//   return(<div className="masterplan-legend">{items.map(({color,label})=><div key={label} className="legend-item"><div className="legend-dot" style={{background:color}}/><span>{label}</span></div>)}</div>);
// }
...
//     if(pid){const opt=mockProjects.find(p=>String(p.id)===pid);setSelProjId(pid);setMpName(opt?.name||'');loadMap(pid);}
//   },[]);// eslint-disable-line
...
//   const handleBuy=code=>alert(`Reserve unit: ${code}`);
...
//   /* ── CHANGE 4: mkClass — glow building pin when its child unit is focused ── */
//   const mkClass=pos=>{
//     let c='unit-marker';
...
//     }
//     // Glow if this pin IS the focused unit, OR if it's a building that CONTAINS the focused unit
...
//         <div className="masterplan-page-select-wrap"><ProjectSelect projects={mockProjects} value={selProjId} onChange={handleProjectChange}/></div>
//       </div>

//       {/* {mapState==='loaded'&&<Legend/>} */}
...
//         if(r.type==='single')return<>{isMobile()&&<MobileBackdrop onClose={()=>setTooltip(null)}/>}<SingleTooltip unit={r.data} companyId={r.company_id} markerRect={markerRect} onClose={()=>setTooltip(null)} onBuy={handleBuy} onViewLayout={setLayoutImages} onShowInCatalog={handleShowInCatalog}/></>;
//         if(r.type==='building')return<>{isMobile()&&<MobileBackdrop onClose={()=>setTooltip(null)}/>}<BuildingTooltip buildingName={r.building_name} units={r.data} companyId={r.company_id} markerRect={markerRect} onClose={()=>setTooltip(null)} onBuy={handleBuy} onViewLayout={setLayoutImages} urlFilters={urlFilterList} frontendFilters={ff} onShowInCatalog={handleShowInCatalog}/></>;
//         return null;
//       })()}
...
// import './masterplans.css';
// import { mockProjects, getMasterplanData, getUnitDetails, subscribeToSettingsChanges } from '../../data/masterplansdata';
...
// /* ── Single Tooltip ── */
// function SingleTooltip({ unit, companyId, markerRect, onClose, onViewLayout, onShowInCatalog }) {
//   const hasBrochure = unit.layout_images?.length > 0;
...
//         {isValidMetric(unit.roof_terraces_area)&&<div className="tooltip-field"><label>Roof/Terrace:</label><span>{formatArea(unit.roof_terraces_area)} m²</span></div>}
//         {hasBrochure&&<button className="buy-now-btn" style={{backgroundColor:'#17a2b8',marginTop:5,marginBottom:5}} onClick={()=>onViewLayout(unit.layout_images)}><i className="fa-regular fa-images"/> View Brochure</button>}
//         {(unit.status==='Available'||IS_MANAGER)&&(
...
// /* ── Building Tooltip ── */
// function BuildingTooltip({ buildingName, units, companyId, markerRect, onClose, onViewLayout, urlFilters, frontendFilters, onShowInCatalog }) {
//   const { selCodes,selBeds,selFinish,selMod,selStat,minA,maxA } = frontendFilters;
//   const visible = units.filter(u=>{
//     if(urlFilters?.length&&!urlFilters.includes(u.unit_code))return false;
//     if(selCodes.length&&!selCodes.includes(u.unit_code))return false;
...
//             <div className="fmr-grp">
//               <label className="fmr-lbl"><i className="fa-solid fa-circle-dot"/> Status {filterSel.stat.length>0&&<span className="fmr-cnt">{filterSel.stat.length}</span>}</label>
//               <MultiSelect options={allOpts.stat} selected={filterSel.stat} onChange={v=>set('stat',v)} placeholder="Select status…" validSet={validSets.stat}/>
//               {allOpts.stat.length===0&&<span className="fmr-empty-txt">—</span>}
//             </div>
//           )}
//         </div>
...
//       {tooltip&&(()=>{
//         const{response:r,markerRect}=tooltip;
//         if(r.type==='single')return<>{isMobile()&&<MobileBackdrop onClose={()=>setTooltip(null)}/>}<SingleTooltip unit={r.data} companyId={r.company_id} markerRect={markerRect} onClose={()=>setTooltip(null)} onViewLayout={setLayoutImages} onShowInCatalog={handleShowInCatalog}/></>;
//         if(r.type==='building')return<>{isMobile()&&<MobileBackdrop onClose={()=>setTooltip(null)}/>}<BuildingTooltip buildingName={r.building_name} units={r.data} companyId={r.company_id} markerRect={markerRect} onClose={()=>setTooltip(null)} onViewLayout={setLayoutImages} urlFilters={urlFilterList} frontendFilters={ff} onShowInCatalog={handleShowInCatalog}/></>;
//         return null;
//       })()}
