...
/* ── Single Tooltip ── */
function SingleTooltip({ unit, companyId, markerRect, onClose, onViewLayout, onShowInCatalog, getLiveImages }) {
...
/* ── Building Tooltip ── */
function BuildingTooltip({ buildingName, units, companyId, markerRect, onClose, onViewLayout, urlFilters, frontendFilters, onShowInCatalog, getLiveImages }) {
  const { selCodes,selBeds,selFinish,selMod,selStat,minA,maxA } = frontendFilters;
  const visible = units.filter(u=>{
    if(urlFilters?.length&&!urlFilters.includes(u.unit_code))return false;
    if(selCodes.length&&!selCodes.includes(u.unit_code))return false;
...
export default function Masterplans() {
  ...
  return(
    <div className="unit-mapping-container">
      ...
      {tooltip&&(()=>{
        const{response:r,markerRect}=tooltip;
        if(r.type==='single')return<>{isMobile()&&<MobileBackdrop onClose={()=>setTooltip(null)}/>}<SingleTooltip unit={r.data} companyId={r.company_id} markerRect={markerRect} onClose={()=>setTooltip(null)} onViewLayout={setLayoutImages} onShowInCatalog={handleShowInCatalog} getLiveImages={getLiveImages}/></>;
        if(r.type==='building')return<>{isMobile()&&<MobileBackdrop onClose={()=>setTooltip(null)}/>}<BuildingTooltip buildingName={r.building_name} units={r.data} companyId={r.company_id} markerRect={markerRect} onClose={()=>setTooltip(null)} onViewLayout={setLayoutImages} urlFilters={urlFilterList} frontendFilters={ff} onShowInCatalog={handleShowInCatalog} getLiveImages={getLiveImages}/></>;
        return null;
      })()}
