...
function ConfigModal({ open, company, onClose, onDone }) {
  ...
  function handleDoneClick() {
    // Native validation UI (no SweetAlert) for required fields
  ...
  return (
    <div className="mu-modal-backdrop" role="dialog" aria-modal="true" onMouseDown={onClose}>
      <div className="mu-modal" onMouseDown={(e) => e.stopPropagation()} style={{ width: "min(900px, 92vw)" }}>
                  ...
                  <div className="cu-two">
                    <label className="mu-label">
                      ...
                      <input
                        ...
                        type="url"
                        required
                        value={erpDraft.mainUrl}
                        onChange={(e) => setErpDraft((p) => ({ ...p, mainUrl: e.target.value }))}
                      />
                    </label>
                    ...
                    <label className="mu-label">
                      ...
                      <input
                        ...
                        type="text"
                        required
                        value={erpDraft.mainKey}
                        onChange={(e) => setErpDraft((p) => ({ ...p, mainKey: e.target.value }))}
                      />
                    </label>
                ...
                <div style={{ marginTop: 14 }}>
                  <label className="mu-label">
                    ...
                    <input
                      ...
                      type="url"
                      required
                      value={gsDraft.sheetUrl}
                      onChange={(e) => setGsDraft((p) => ({ ...p, sheetUrl: e.target.value }))}
                    />
                  </label>
                  ...
                  <div className="cu-two" style={{ marginTop: 12 }}>
                    <label className="mu-label">
                      GID *
                      <input className="mu-field" type="text" required value={gsDraft.gid} onChange={(e) => setGsDraft((p) => ({ ...p, gid: e.target.value }))} />
                    </label>

                    <label className="mu-label">
                      Title *
                      <input className="mu-field" type="text" required value={gsDraft.title} onChange={(e) => setGsDraft((p) => ({ ...p, title: e.target.value }))} />
                    </label>
                  </div>
                </div>
              </>
          ...
          <form ref={formRef}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 10 }}>
              <div className="mc-logo">
                {company.logoUrl ? (
                  <img src={company.logoUrl} alt="logo" className="mc-logo-img" />
                ) : (
                  <div className="mc-logo-ph">No logo</div>
                )}
              </div>

              <label className="mc-upload">
                <input type="file" accept="image/*" onChange={(e) => onUploadLogo(e.target.files?.[0] || null)} style={{ display: "none" }} />
                Upload Logo
              </label>
            </div>
            ...
            <div className="mc-mobile-actions" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 16 }}>
...
export default function ManageCompanies() {
  ...
  // MOBILE SYNC FLOW (exactly as requested)
  async function handleSyncMobileFlow(c) {
    ...
    if (!res.isConfirmed) {
      // user cancelled: show form again
      setMobileEdit({ open: true, companyId: c.id });
      return;
    }

    // 3) show edit form again + spinner (syncing=true)
    setMobileEdit({ open: true, companyId: c.id });
    setSyncing((p) => ({ ...p, [c.id]: true }));
    ...
    try {
      ...
      // 4) after syncing, hide form and show success sweetalert
      setMobileEdit({ open: false, companyId: null });
      await wait(60);
  ...
  async function handleLogoChange(c, file) {
    if (!file) return;
  ...
  // mobile config re-open edit after done/cancel
  ...
  return (
    <div className="lux-page manage-companies">
                ...
                <tr key={c.id}>
                  ...
                  <td>
                    <div className="mc-logo-wrap">
                      <div className="mc-logo">
                        {c.logoUrl ? <img src={c.logoUrl} alt="logo" className="mc-logo-img" /> : <div className="mc-logo-ph">No logo</div>}
                      </div>

                      <label className="mc-upload">
                        <input type="file" accept="image/*" onChange={(e) => handleLogoChange(c, e.target.files?.[0] || null)} style={{ display: "none" }} />
                        Upload
                      </label>
                    </div>
                  </td>
      ...
      <MobileEditModal
        ...
        onUploadLogo={(file) => mobileCompany && handleLogoChange(mobileCompany, file)}
