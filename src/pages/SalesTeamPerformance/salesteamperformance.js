  ...
  return (
    <>
      {rows.map((row) => {
        ...
        // Build children rows for expanded teams
        let childRows = [];
...
export default function SalesTeamPerformance() {
  ...
  const currentData = currentCompany ? COMPANY_DATA[currentCompany] : [];

  // ── Memoized filteredData — only recomputes when filters/company/dates actually change
  // This prevents charts from re-rendering when merely opening/closing a dropdown
  const filteredData = useMemo(
    () => (currentCompany ? applyFilters(currentData, selectedFilters, startDate, endDate) : []),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [currentCompany, selectedFilters, startDate, endDate]
  );
    ...
    <div className="stp-root">
      ...
      <div className="stp-header">
        <div className="stp-header-left">
          <h1>Sales Requests Analysis Dashboard</h1>
        </div>
      ...
      <div className="stp-container">
        {!currentCompany ? (
          <div className="stp-welcome">
            ...
            <p>Select a company to view the sales requests analysis dashboard</p>
          </div>
        ) : (
