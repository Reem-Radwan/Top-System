...
//   if (role === "BusinessTeam") return "Business Team";
//   return role || "";
// }
...
//   const [searchCompany, setSearchCompany] = useState(""); // only used in Companies view
//   const [searchRole, setSearchRole] = useState("");

//   // collapse groups by role
//   const [collapsed, setCollapsed] = useState({}); // { [role]: boolean }

//   // last values (to show Swal only when value really changed)
...
//     confirmButtonColor: "#f59e0b", // optional (light orange)
//     allowOutsideClick: false,      // optional (forces clicking OK)
//     allowEscapeKey: true,          // optional
//   });
// }
...
//       // no toast here (SweetAlert handles UX now)
//       return payload;
//     } catch {
//       Swal.fire({ icon: "error", title: "Error", text: "Connection error. Could not save." });
//       return null;
//     }
//   }

//   // Generic “field edited” handler: show Swal only if changed vs lastValuesRef, then save.
...
//     await triggerAjaxSave(userId);
//   }

//   // ----- View data sets -----
//   const companiesUsers = useMemo(
//     () => users.filter((u) => ["CompanyController", "CompanyUser", "Manager" , "Finance"].includes(u.role)),
//     [users]
//   );

//   const adminBtUsers = useMemo(
//     () => users.filter((u) => ["Admin", "BusinessTeam"].includes(u.role)),
//     [users]
//   );
...
//   // ----- Group by role for collapsible sections -----
...
//     if (view === VIEW.COMPANIES) return ["CompanyController", "CompanyUser", "Manager"];
//     return ["Admin", "BusinessTeam"];
//   }, [view]);
...
//             onClick={() => setView(VIEW.COMPANIES)}
//           >
//             Companies
//           </button>
//           <button
...
//                           {/* Companies view: show Can Edit / Can Change Years; Admin view hides them */}
//                           {view === VIEW.COMPANIES ? <th>Can Edit</th> : null}
//                           {view === VIEW.COMPANIES ? <th>Can Change Years</th> : null}

//                           <th>Company</th>

//                           {/* Admin/BT view: Job title input; Companies view hides job title */}
//                           {view === VIEW.ADMIN_BT ? <th>Job Title</th> : null}
...
//                                   onBlur={() => triggerAjaxSave(u.id)} // keep same behavior: save on blur
//                                 />
//                               </td>
...
//                                     onFieldCommit(u.id, "companyName", "Company", nextName); // SweetAlert2 + save
//                                   }}
//                                 >
//                                   <option value="">Select company</option>
//                                   {["Mint", "Palmier Developments", "IGI Developments"].map((n) => (
...
//   roleGroup,
//   rolesConfig,
//   updateDraft,         // (patch) => void (only local draft)
//   onSaveClick,         // () => Promise<void>  (the "Save icon" behavior)
//   onFieldCommit,       // async (fieldKey, label, value) => void
// }) {
//   if (!open || !user) return null;

//   const cfg = rolesConfig(roleGroup, view);

//   // In modal, allow editing all fields for that user (based on current view/role)
//   // but commit via the same onFieldCommit logic (so same SweetAlerts & saving rules).
...
//   return (
//     <div className="mu-modal-backdrop" role="dialog" aria-modal="true">
//       <div className="mu-modal">
//         <div className="mu-modal-header">
//           <div className="mu-modal-title">User Details</div>
//           <button className="mu-modal-x" type="button" onClick={onClose}>
//             ✕
//           </button>
//         </div>
...
//   // collapse groups by role (note: default closed handled in render with ?? true)
//   const [collapsed, setCollapsed] = useState({});

//   // mobile: show only first column + "Show more" modal
...
//   }, []); // matchMedia change event supported via addEventListener("change", ...) [web:97]
...
//       return payload;
//     } catch {
//       await Swal.fire({ icon: "error", title: "Error", text: "Connection error. Could not save." });
//       return null;
//     }
//   }

//   // Generic “field edited” handler: show Swal only if changed vs lastValuesRef, then save (without password)
//   async function onFieldCommit(userId, fieldKey, fieldLabel, newValue) {
//     const prev = lastValuesRef.current[userId]?.[fieldKey];
...
//   // View data sets
//   const companiesUsers = useMemo(
//     () => users.filter((u) => ["CompanyController", "CompanyUser", "Manager", "Finance Manager"].includes(u.role)),
//     [users]
//   );

//   const adminBtUsers = useMemo(
//     () => users.filter((u) => ["Admin", "BusinessTeam" , "Developer"].includes(u.role)),
//     [users]
//   );
...
//   // Group by role for collapsible sections, with requested ordering
//   const grouped = useMemo(() => {
//     const map = new Map();
//     for (const u of filtered) {
//       const key = u.role || "Unknown";
...
//   const rolesInThisView = useMemo(() => {
//     if (view === VIEW.COMPANIES) return ["CompanyUser", "CompanyController", "Finance Manager", "Manager"];
//     return ["Admin", "BusinessTeam" , "Developer"];
//   }, [view]);

//   // Per-role/table rules
...
//     return {
//       showCanEditCols: isCompaniesView && roleGroup === "CompanyUser", // only in CompanyUser group
//       showCompanyCol: isCompaniesView, // Prometheus: NO company for Admin or BusinessTeam
//       showJobTitleCol: isPrometheusView && isBusinessTeamGroup, // Admin: remove job title
//       adminAlwaysActive: isAdminGroup, // Admin always active & not editable
//     };
//   }
...
//           const isCollapsed = collapsed[role] ?? true; // 1) default closed

//           return (
//             <div className="lux-group" key={role}>
//               <button type="button" className="lux-group-header" onClick={() => toggleRole(role)}>
//                 <span className="lux-group-role">{displayRole(role)}</span>
//                 <span className="lux-group-meta">{roleUsers.length} user{roleUsers.length === 1 ? "" : "s"}</span>
//               </button>

//               {!isCollapsed ? (
//                 <div className="lux-tablewrap">
//                   {/* 6) Mobile: no horizontal scroll, show first column + show more */}
//                   {isMobile ? (
...
//                             <input
//                               className="lux-cell-input mu-mobile-name"
//                               value={u.full_name || ""}
//                               onChange={(e) => updateUser(u.id, { full_name: e.target.value })}
//                               onBlur={(e) => onFieldCommit(u.id, "full_name", "Name", e.target.value)}
//                             />
...
//                             {/* 4) Companies: Can Edit / Can Change Years only for CompanyUser group */}
//                             {cfg.showCanEditCols ? <th>Can Edit</th> : null}
//                             {cfg.showCanEditCols ? <th>Can Change Years</th> : null}

//                             {/* 3) Prometheus: remove company (Admin + Business Team) */}
//                             {cfg.showCompanyCol ? <th>Company</th> : null}

//                             {/* 3) Prometheus: Admin has no job title; Business Team keeps job title */}
...
//                                   value={u.passwordDraft || ""}
//                                   onChange={(e) => updateUser(u.id, { passwordDraft: e.target.value })}
//                                   /* 5) password is NOT saved on blur anymore */
//                                 />
//                               </td>
...
//                               <td data-label="Active">
//                                 {/* 3) Admin always active and can't be deactivated */}
//                                 {cfg.adminAlwaysActive ? (
...
//                                       onChange={(e) => {
//                                         updateUser(u.id, { can_change_years: e.target.checked });
//                                         onFieldCommit(u.id, "can_change_years", "Can Change Years", e.target.checked);
//                                       }}
//                                     />
//                                   ) : (
//                                     <span className="lux-dash">—</span>
...
//                               <td data-label="Actions">
//                                 {/* 5) Save icon applies password if provided + shows alert */}
//                                 <button
//                                   type="button"
//                                   className="btn btn-sm lux-btn"
//                                   title="Save"
...
// /**
//  * Canonicalize role strings so new/typo casing is handled:
//  * - "DeVeloper" -> "Developer"
//  * - "Finance Manager" -> "Finance Manager"
//  * - "CompanyUser" -> "CompanyUser" (kept as canonical id)
//  */
...
//   // fallback: keep original but trimmed
//   return (role || "").trim();
// }

// /**
//  * Display role nicely in UI:
...
//   // CamelCase -> spaced (for any future roles)
...
//   // collapse groups by role (default closed done in render)
//   const [collapsed, setCollapsed] = useState({});
...
//   // View data sets (use canonicalRole so "DeVeloper" works)
//   const companiesUsers = useMemo(
...
//   // Filtering (role filter uses cleanRole, so spaces/case don't matter)
//   const filtered = useMemo(() => {
//     const nameF = (searchName || "").toLowerCase().trim();
//     const roleF = cleanRole(searchRole || "");
//     const companyF = (searchCompany || "").toLowerCase();
...
//   // Group by role for collapsible sections, with correct ordering
//   const grouped = useMemo(() => {
//     const map = new Map();
//     for (const u of filtered) {
...
//     return {
//       // Can Edit/Change Years: only for CompanyUser group (as you requested)
//       showCanEditCols: isCompaniesView && roleGroup === "CompanyUser",
//       // Company column exists only in Companies table
//       showCompanyCol: isCompaniesView,
//       // Job Title exists only for Business Team group in Prometheus table
...
//         {grouped.map(([role, roleUsers]) => {
//           const cfg = rolesConfig(role, view);
//           const isCollapsed = collapsed[role] ?? true; // default closed
...
//               {!isCollapsed ? (
//                 <div className="lux-tablewrap">
//                   {/* Mobile: show first column + show more modal */}
//                   {isMobile ? (
//                     <div className="mu-mobile-list">
//                       {roleUsers.map((u) => (
//                         <div className="mu-mobile-item" key={u.id}>
...
//                                 <td data-label="Role">
//                                   {/* Companies role display now has spaces (Company User, etc.) */}
//                                   <span className="lux-pill">{displayRole(u.role)}</span>
//                                 </td>
...
function UserDetailsModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      onMouseDown={onClose} // click anywhere outside closes
    >
...
export default function ManageUsers() {
  ...
  // SweetAlert defaults: click outside + ESC should close [web:85]
  const swalFire = (opts) =>
    Swal.fire({
  ...
  function toggleRole(role) {
    const current = collapsed[role] ?? true;
    setCollapsed((prev) => ({ ...prev, [role]: !current }));
  }
                    ...
                    <div className="mu-mobile-list">
                      {roleUsers.map((u) => (
                        <div className="mu-mobile-item" key={u.id}>
                          ...
                          {/* Mobile: remove outside Save button */}
                          <div className="mu-mobile-actions">
                            <button
                              type="button"
                              className="btn btn-sm lux-btn danger"
