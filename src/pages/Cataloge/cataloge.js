...
export default function Catalog(){
  ...
  // A counter that bumps whenever UnitBrochureManager uploads/deletes/reorders
  // images. This forces a re-render so getLiveImages() picks up the new data.
  ...
  /**
   * Returns live layout_images for a unit, preferring the bridge store.
   * Bridge returns { id, url, label }[] — we extract plain URL strings
   * so the rest of the component (hasBrochure check, LayoutModal) is unchanged.
   */
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // brochureTick is intentionally listed so the callback re-creates when images change
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[brochureTick]);
  ...
  return(
    <div className="App" id="catalog">
      ...
      {/* ── FIX 3: Back banner now rendered — consumes retUrl + handleBackToMap ── */}
       {retUrl && (
        <div className="back-banner">
          <button type="button" onClick={handleBackToMap}>
