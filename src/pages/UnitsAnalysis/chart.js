              ...
              <div className="chart-visualization" style={{ width: `${totalWidth}px`, minWidth: `${totalWidth}px` }}>
                ...
                <div style={{ display: 'flex', width: '100%', paddingTop: '40px', paddingBottom: '40px' }}>
                  ...
                  {flattenedColumns.map((item, index) => {
                    ...
                    const location = item.fullData?.locations || item.fullData?.Location || item.fullData?.location || '—';
                    const assetType = selectedFilters?.assetType?.length 
                      ? selectedFilters.assetType.join(', ')
                      : item.fullData?.['Asset Type'] || item.fullData?.assetType || '—';
                    const unitType = selectedFilters?.unitType?.length 
                      ? selectedFilters.unitType.join(', ')
                      : item.fullData?.['Unit Type'] || item.fullData?.unitType || '—';
                      ...
                      >
                        <div className="chart-stack-wrapper">
                          ...
                          <div className="chart-tooltip">
                            ...
                            <div className="tooltip-item"><span className="tooltip-label">Location:</span> {location}</div>
                            <div className="tooltip-item"><span className="tooltip-label">Asset Type:</span> {assetType}</div>
                            <div className="tooltip-item"><span className="tooltip-label">Unit Type:</span> {unitType}</div>
                          </div>
                        </div>
                      </div>
                    );
