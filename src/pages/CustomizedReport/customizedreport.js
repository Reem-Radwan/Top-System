...
// import { MOCK } from "../../data/customizedreportdata";

// // ── INJECT FONT AWESOME ONCE (scoped, won't duplicate) ──
...
// // ── PIVOT TABLE (renders HTML string via dangerouslySetInnerHTML for full fidelity) ──
...
//     // Only render sub-header row when there are multiple measures (shows measure names under each column group)
//     // BUT suppress it when columns exist - the repeated measure names per column are confusing
//     // Only show it when NO columns (measures-only header)
//   } else {
//     html += `<tr>`;
//     for (let ci = 0; ci < groupColsCount; ci++) {
//       html += `<th class="crpt-sticky-col">${esc(config.rows[ci] ? labelOf(fieldsMap, config.rows[ci]) : "")}</th>`;
...
// // ── APPLY STICKY ROW/COL POSITIONS ──
// function applyStickyRowCols(tableEl, count) {
//   if (!tableEl) return;

//   // Reset all previously set sticky styles
//   tableEl.querySelectorAll(".crpt-sticky-col").forEach(el => {
//     el.style.left = "";
//     el.style.zIndex = "";
//     el.style.position = "";
//   });

//   // Sticky LEFT columns (row label columns)
//   let left = 0;
//   for (let ci = 1; ci <= count; ci++) {
//     const sample = tableEl.querySelector(`tbody tr td:nth-child(${ci})`) ||
...
//       if (cell.closest(".crpt-gt-row")) {
//         cell.style.zIndex = 560;
//       } else {
...
//   // Sticky RIGHT columns (grand total column group) — measure widths from first data row
//   // Collect gt-col-sticky cells from thead row 1 to get column count & widths
//   const firstThead = tableEl.querySelector("thead tr");
//   if (firstThead) {
//     const gtCells = Array.from(firstThead.querySelectorAll(".crpt-gt-col-sticky"));
//     if (gtCells.length) {
//       // Compute cumulative right offsets from the right edge
//       const widths = gtCells.map(c => c.getBoundingClientRect().width || 80);
//       const rights = [];
//       let acc = 0;
...
//       // Apply to every row in the table
//       tableEl.querySelectorAll("tr").forEach(row => {
//         const rowGtCells = Array.from(row.querySelectorAll(".crpt-gt-col-sticky"));
//         if (!rowGtCells.length) return;
//         rowGtCells.forEach((cell, idx) => {
//           cell.style.position = "sticky";
//           cell.style.right = (rights[idx] ?? 0) + "px";
//         });
//       });
//     }
//   }
...
//   const [fpDraft, setFpDraft] = useState({}); // {val: bool}
...
//     setTimeout(() => {
//       const data = MOCK[id];
//       if (!data) { setErrorMsg("Company not found."); setLoading(false); return; }
//       setRawUnits(data.units);
//       setFieldsMeta(data.fields);
//       const fm = {};
//       data.fields.forEach(f => { fm[f.name] = f; });
//       setFieldsMap(fm);
//       setConfig({ rows: [], cols: [], filters: [], values: [], filterValues: {} });
//       setStatusText(`Loaded ${data.count} units • ${data.company.name}`);
//       setMetaText(`${data.company.name} • ${data.count} records`);
//       setMetaLine(data.company.name);
//       setLoading(false);
//     }, 200);
//   }
...
//   // ── CHIPS REMOVE ──
//   function handleRemoveRCF(fn) {
//     setConfig(prev => removeFieldFromRCF(prev, fn));
//   }

//   function handleRemoveValue(id) {
//     setConfig(prev => ({ ...prev, values: prev.values.filter(x => x.id !== id) }));
//   }

//   function handleAggChange(id, agg) {
//     setConfig(prev => ({
//       ...prev,
//       values: prev.values.map(m => m.id === id ? { ...m, agg } : m),
//     }));
//   }
...
//     const draft = {};
//     allVals.forEach(v => {
//       draft[v] = !chosen || (chosen instanceof Set && chosen.has(v));
//     });
//     setFpField(fn);
...
//       filterValues: {
//         ...prev.filterValues,
//         [fpField]: sel.size === allVals.length ? null : sel,
//       },
//     }));
//     closeFilterPopup();
//   }
...
//     setConfig(prev => ({
//       ...prev,
//       filterValues: { ...prev.filterValues, [fpField]: new Set() },
//     }));
//     closeFilterPopup();
//   }
...
//               value={companyId}
//               onChange={e => { setCompanyId(e.target.value); loadCompany(e.target.value); }}
//             >
//               <option value="">-- Select Company --</option>
//               <option value="mint">Mint Developments</option>
//               <option value="palmier">Palmier Developments</option>
//               <option value="igi">IGI Developments</option>
//             </select>
//           </div>
//           <button
...
//                   type="checkbox"
//                   checked={grandTotals}
//                   onChange={e => setGrandTotals(e.target.checked)}
//                 />
//                 Grand Totals
//               </label>
//               <label className="crpt-toggle-label">
//                 <input
//                   type="checkbox"
//                   checked={percentOfGrand}
//                   onChange={e => setPercentOfGrand(e.target.checked)}
//                 />
//                 % of Grand Total
//               </label>
//               <button
...
//             {/* Error Box */}
//             {errorMsg && (
//               <div className="crpt-error-box crpt-visible">{errorMsg}</div>
//             )}
...
//                       <div
//                         key={fn}
//                         className="crpt-rf-item"
//                         onClick={e => openFilterPopup(fn, e.currentTarget)}
//                       >
...
//                         {!rawUnits.length
//                           ? "No units loaded yet."
//                           : "Configure Rows / Columns / Filters / Values to render the pivot."}
//                       </div>
//                     )}
//                     <div
//                       ref={pivotHostRef}
//                       dangerouslySetInnerHTML={{ __html: pivotHtml }}
//                     />
//                   </div>
//                 </div>
//               </div>
...
//                       <div
//                         className="crpt-area"
//                         onDrop={e => handleAreaDrop(e, "filters")}
...
//                       <div
//                         className="crpt-area"
//                         onDrop={e => handleAreaDrop(e, "cols")}
//                         onDragOver={handleAreaDragOver}
//                       >
...
//                       <div
//                         className="crpt-area"
//                         onDrop={e => handleAreaDrop(e, "rows")}
//                         onDragOver={handleAreaDragOver}
//                       >
...
//                       <div
//                         className="crpt-area"
//                         onDrop={e => handleAreaDrop(e, "values")}
//                         onDragOver={handleAreaDragOver}
//                       >
...
//                                   className="crpt-agg"
//                                   value={m.agg}
//                                   onChange={e => handleAggChange(m.id, e.target.value)}
//                                 >
//                                   {allowed.map(a => (
...
//       <div
//         ref={filterPopupRef}
//         className={`crpt-filter-popup${fpOpen ? " crpt-visible" : ""}`}
//         style={{ top: fpPos.top, left: fpPos.left }}
//       >
...
//                   type="checkbox"
//                   checked={!!fpDraft[v]}
//                   onChange={e => setFpDraft(prev => ({ ...prev, [v]: e.target.checked }))}
//                 />
//                 <span>{v}</span>
//               </div>
//             ))}
...
for (let ci = 1; ci <= count; ci++) {
    const sample =
      ...
      tableEl.querySelector(`thead tr th:nth-child(${ci})`);
    const w = sample ? sample.getBoundingClientRect().width : 120;
    const currentLeft = left;   // ← snapshot into a const — safe to close over

    tableEl.querySelectorAll(`tr > *:nth-child(${ci})`).forEach(cell => {
      cell.classList.add("crpt-sticky-col");
      cell.style.position = "sticky";
      cell.style.left     = currentLeft + "px";   // ← uses the snapshot
      cell.style.zIndex   = cell.closest(".crpt-gt-row")
        ? 560
        : cell.tagName === "TH" ? 7 : 5;
    });
    left += w;
  ...
  // ── STICKY RIGHT (grand-total columns) ──
  // Read per-cell widths from a regular body row — NOT from thead,
  // because the thead may have a single spanning <th> for the whole GT group.
  ...
  // Build right-offset array: rightmost cell → right:0, next → right:widthOfRightmost, …
  const widths = gtBodyCells.map(c => c.getBoundingClientRect().width || 80);
  const rights = new Array(widths.length);
  let acc = 0;
  for (let i = widths.length - 1; i >= 0; i--) {
    rights[i] = acc;
    acc += widths[i];
  ...
  tableEl.querySelectorAll("tr").forEach(row => {
    ...
    const inGtRow = row.classList.contains("crpt-gt-row");

    rowGtCells.forEach((cell, idx) => {
      cell.style.position = "sticky";

      // A spanning <th> (colspan > 1) covers the whole GT zone → pin flush-right
      const isSpanningHeader =
        cell.tagName === "TH" &&
        parseInt(cell.getAttribute("colspan") || "1") > 1;

      cell.style.right  = isSpanningHeader ? "0px" : (rights[idx] ?? 0) + "px";
      cell.style.zIndex = inGtRow ? 620 : cell.tagName === "TH" ? 20 : 15;
    });
  });
}
