...
export default function DualPayment() {
  ...
  const reloadPayments = useCallback(() => {
    ...
    const res = mockFetch(project, year, scheme, code);
    if (res.success) {
      const data = res.data;
  ...
  function handleDeletePlan() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = mockDelete(project, year, scheme, planCode);
  ...
  // ─── Disable Additional Discount ───
  function handleDisableChange(e) {
    const val = e.target.checked;
    setDisableAdditional(val);
    if (!project) return;
    mockSave({ project_id: project, year, scheme, payment_plan_code: planCode, disable_additional_discount: val });
  }
            ...
            <div className="dp__filters-grid">
              ...
              <div className="dp__field">
                ...
                <select value={company} onChange={handleCompanyChange}>
                  ...
                  <option value="alpha">Alpha Developments</option>
                  <option value="beta">Beta Real Estate</option>
                  <option value="gamma">Gamma Properties</option>
                  <option value="delta">Delta Holdings</option>
                </select>
              </div>
              ...
              <div className="dp__field">
                ...
                <select value={project} onChange={e => setProject(e.target.value)} disabled={!company}>
                  ...
                  {company && COMPANY_PROJECTS[company]?.map(p => (
