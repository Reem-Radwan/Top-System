...
export default function CreateCompany() {
  ...
  const payload = useMemo(() => {
    return {
      ...
      logo: logoFile || null,
      gsheet: isGS
        ? { sheetUrl: gs.sheetUrl, gid: gs.gid || null, title: gs.title || null }
        : null,
      erp: isERP ? { ...erp } : null,
    };
  }, [name, type, joiningDate, active, logoFile, isGS, isERP, gs, erp]);
  ...
  async function handleSubmit(e) {
    ...
    // console.log(payload); // for backend later
  }
    ...
    <div className="lux-page create-company-page">
      ...
      <div className="cc-card">
        <form ref={formRef} onSubmit={handleSubmit}>
          <div className="cc-grid">
            ...
            {/* Joining date */}
             {/* Joining date */}
              ...
              {/* Upload logo (now beside Joining Date) */}
              <label className="mu-label">
                Upload Logo
                <input
                  className="mu-field"
          ...
          {/* remove later: */}
          {/* <pre style={{ marginTop: 14 }}>{JSON.stringify(payload, null, 2)}</pre> */}
        </form>
      </div>
    </div>
  );
