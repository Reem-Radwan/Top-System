...
//   // Analysis data
//   const [priceRangeData, setPriceRangeData]   = useState(null);
//   const [unitTypeData, setUnitTypeData]        = useState(null);   // NEW from HTML
//   const [unitModelData, setUnitModelData]      = useState(null);
//   const [premiumData, setPremiumData]          = useState({});
...
//   // ──────────────────────────────────────────────────────────
//   // DOWNLOAD (Excel) – enriched like HTML version
//   // ──────────────────────────────────────────────────────────
//   const handleDownloadAll = async () => {
//     if (!hasData) {
//       showToast('warning', 'No data to download');
...
//       const addMeta = (ws) => {
//         // Append generated-on and project rows (from HTML)
...
//   // FORMATTERS
//   // ──────────────────────────────────────────────────────────
//   // Matches HTML version: adds "M EGP" / "K EGP" suffix
//   const formatPriceEGP = (price) => {
...
//   const formatPremiumType = (type) => {
//     const mapping = { main_view:'Main View', secondary_view:'Secondary View', back_view:'Back View', levels:'Levels', north_breeze:'North Breeze', corners:'Corners', accessibility:'Accessibility' };
//     return mapping[type] || type;
//   };
...
//   // ──────────────────────────────────────────────────────────
//   // GRAND TOTAL ROW (standard tables)
//   // ──────────────────────────────────────────────────────────
//   const renderGrandTotalRow = (totals) => {
//     const totalUnreleased        = (totals.all || 0) - (totals.released || 0);
//     const totalUnreleasedPercent = totals.all > 0 ? (totalUnreleased / totals.all) * 100 : 0;
//     const totalReleasedPercent   = totals.all > 0 ? (totals.released / totals.all) * 100 : 0;
...
//   // ──────────────────────────────────────────────────────────
//   // RENDER: UNIT TYPE TABLE (NEW – from HTML)
//   // ──────────────────────────────────────────────────────────
...
//   // ──────────────────────────────────────────────────────────
//   // RENDER: PREMIUM TABLE (expanded column structure from HTML)
//   // ──────────────────────────────────────────────────────────
//   const renderPremiumTable = (premiumType) => {
//     const currentData = premiumData[premiumType];
...
//   // Available premium types that actually have data
...
//             {/* Company selector */}
//             <div className="spa-selector-wrapper">
//               <label className="spa-selector-label">Company</label>
//               <div className="spa-selector-container">
//                 {isRestrictedUser ? (
//                   <div className="spa-static-value" style={{
//                     padding: '10px 15px',
...
//             {/* Project selector – shown when company is selected */}
...
//           {/* Right actions – hidden until a project is selected (from HTML) */}
//           <div className="spa-header-right">
//             {selectedProject && (
//               <>
//                 <button className="spa-header-btn spa-btn-secondary" onClick={handleReset}>
...
//                     { key: 'unitType',   label: 'Unit Type' },    // NEW
...
//   // ─── Load companies on mount ───────────────────────────────
//   // FIX: Wrapped in useCallback so the function reference is stable,
//   // allowing it to be safely added to the useEffect dependency array
...
//       showToast('danger', 'Failed to load companies');
//     }
//   }, []); // no external deps — salesPerformanceService is module-level
