...
//   if (hasExpanded) {
//     // Collapse all - set everything to false
//     setExpandedCities({});
//     setExpandedProjects({});
//   } else {
//     // Expand all - set everything to true
...
//                 <th key={month} className="month-header" colSpan={[visibleColumns.percentage, visibleColumns.noOfUnits, visibleColumns.salesValue].filter(Boolean).length}>
//                   {getMonthName(month)} {activeYear}
//                 </th>
//               ))}
//             </tr>
...
//                   {visibleColumns.percentage && <th className="sub-header">%</th>}
...
//               const cityYearData = city.yearData[activeYear];
...
//                     </td>
//                     {activeYearMonths.map(month => {
//                       const monthData = cityYearData?.months[month];
//                       const count = monthData?.count || 0;
//                       const value = monthData?.value || 0;
//                       const total = cityYearData?.total || 0;

//                       return (
//                         <React.Fragment key={month}>
//                           {visibleColumns.percentage && <td className="metric-cell">{formatPercentage(count, total)}</td>}
...
//                     const isProjectExpanded = expandedProjects[projectKey]=== true;
//                     const projectYearData = project.yearData[activeYear];
...
//                           </td>
//                           {activeYearMonths.map(month => {
//                             const monthData = projectYearData?.months[month];
//                             const count = monthData?.count || 0;
//                             const value = monthData?.value || 0;
//                             const total = projectYearData?.total || 0;
...
//                           const typeYearData = unitType.yearData[activeYear];
...
//                               </td>
//                               {activeYearMonths.map(month => {
//                                 const monthData = typeYearData?.months[month];
//                                 const count = monthData?.count || 0;
//                                 const value = monthData?.value || 0;
//                                 const total = typeYearData?.total || 0;
...
const SalesProgressPivot = ({ units }) => {
  ...
  // Selected periods for comparison mode
  ...
  const progressData = useMemo(() => {
    ...
    if (years.length > 0 && activeYear === null && !comparisonMode) {
      setActiveYear(years[0]);
    }

    return { cityData: Object.values(cityMap), years };
  }, [units, activeYear, comparisonMode]);
  ...
  // Active display months (either year months or comparison periods)
  const displayMonths = comparisonMode ? comparisonPeriods : 
  ...
  const updatePeriodMonths = (index, months) => {
    const newPeriods = [...selectedPeriods];
    newPeriods[index].months = months;
    setSelectedPeriods(newPeriods);
  };
  ...
  const addPeriod = () => {
    if (selectedPeriods.length < 6) { // Max 6 periods
      setSelectedPeriods([...selectedPeriods, { year: null, months: [] }]);
    }
  };

  const removePeriod = (index) => {
    if (selectedPeriods.length > 2) { // Keep at least 2 periods
  ...
  const toggleCity = (cityName) => setExpandedCities(prev => ({ ...prev, [cityName]: !prev[cityName] }));
  const toggleProject = (cityName, projectName) => {
    const key = `${cityName}-${projectName}`;
    setExpandedProjects(prev => ({ ...prev, [key]: !prev[key] }));
  };
  ...
  return (
    <div className="sales-progress-container">
      ...
      {/* Period Selector (Comparison Mode) */}
      ...
      <div className="sales-progress-scroll">
        <table className="sales-progress-table">
            ...
            <tr className="month-header-row">
              ...
              {displayMonths.map((period, index) => (
                <th 
                  ...
                  className={`month-header ${index > 0 && comparisonMode ? 'month-separator' : ''}`}
            ...
            <tr className="column-header-row">
              {displayMonths.map((period, index) => (
                <React.Fragment key={`${period.year}-${period.month}-${index}`}>
                  {visibleColumns.percentage && <th className={`sub-header ${index > 0 && comparisonMode ? 'month-separator' : ''}`}>%</th>}
                  {visibleColumns.noOfUnits && <th className="sub-header">NO. OF UNITS</th>}
                  {visibleColumns.salesValue && <th className="sub-header">SALES VALUE</th>}
                </React.Fragment>
              ))}
            </tr>
          </thead>
          <tbody>
            {visibleRows.cities && progressData.cityData.map(city => {
              const isCityExpanded = expandedCities[city.name] !== false;
                    ...
                    {displayMonths.map((period, periodIndex) => {
                      ...
                      return (
                        <React.Fragment key={`${period.year}-${period.month}-${periodIndex}`}>
                          {visibleColumns.percentage && <td className={`metric-cell ${periodIndex > 0 && comparisonMode ? 'month-separator' : ''}`}>{formatPercentage(count, total)}</td>}
                          {visibleColumns.noOfUnits && <td className="metric-cell">{formatNumber(count)}</td>}
                          {visibleColumns.salesValue && <td className="metric-cell">{formatNumber(value)}</td>}
                        </React.Fragment>
                      );
