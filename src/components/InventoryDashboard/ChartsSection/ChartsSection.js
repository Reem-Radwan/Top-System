...
//   // Pie options for modal - simpler, uses data directly
//   const modalPieOptions = {
//     responsive: true,
//     maintainAspectRatio: false,
//     plugins: {
...
//             // Get sales value directly from chart data
//             let totalSalesValue = 0;
//             if (context.chart.config.data.salesByLabel) {
//               totalSalesValue = context.chart.config.data.salesByLabel[label] || 0;
//             }
...
//     // Calculate sales value by label for tooltips - USE ALLUNITS for complete data
//     const salesByLabel = {};
//     allUnits.forEach(unit => {
//       const status = unit.status || 'Unknown';
//       if (status !== 'Unknown') {
...
//     // Calculate sales value by type for tooltips - USE ALLUNITS for complete data
//     const salesByLabel = {};
//     allUnits.forEach(unit => {
//       const type = unit.unit_type || 'Unknown';
//       if (type !== 'Unknown') {
...
//             // Use allUnits to get complete sales data
//             if (context.chart.canvas.id === 'inventory-chart') {
//               totalSalesValue = allUnits
//                 .filter(u => u.status === label)
//                 .reduce((sum, u) => sum + (parseFloat(u.sales_value) || 0), 0);
...
const ChartsSection = ({
  ...
  // Pie options for modal (FIXED: reads sales from dataset.salesByLabel)
  const modalPieOptions = {
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      ...
      tooltip: {
        callbacks: {
          ...
          label: (context) => {
            ...
            // ✅ Works in modal because we store salesByLabel inside the dataset
            const salesMap = context.dataset.salesByLabel || {};
            const totalSalesValue = salesMap[label] || 0;
          ...
          footer: (context) =>
            `${context[0].label}: ${context[0].parsed} units`,
        },
      },
    },
  };
            ...
            .map((u) => {
              switch (filterType) {
                ...
                case "contractPaymentPlans":
                  return u.adj_contract_payment_plan;
                default:
                  return null;
              }
            })
      ...
      units.forEach((unit) => {
        ...
        try {
          ...
          adjustedContractDate.setMonth(
            adjustedContractDate.getMonth() +
              (parseInt(unit.grace_period_months) || 0),
          );
  ...
  // Inventory chart data (FIXED: salesByLabel moved into dataset)
  const inventoryChartData = useMemo(() => {
    const statusCounts = groupByStatus(units);
    const labels = Object.keys(statusCounts);
    const data = Object.values(statusCounts);
    ...
    // Calculate sales value by status from allUnits
    const salesByLabel = {};
    allUnits.forEach((unit) => {
      const status = unit.status || "Unknown";
      if (status !== "Unknown") {
        salesByLabel[status] =
          (salesByLabel[status] || 0) + (parseFloat(unit.sales_value) || 0);
      }
    });
      ...
      datasets: [
        {
          ...
          borderWidth: 2,
          borderColor: "#fff",
          salesByLabel, // ✅ important for modal tooltip
        },
      ],
    };
  }, [units, allUnits]);

  // Unit type chart data (FIXED: salesByLabel moved into dataset)
  const unitModelChartData = useMemo(() => {
    const typeCounts = groupByUnitType(units);
    const colors = [
      "#FF6384",
    ...
    // Calculate sales value by type from allUnits
    const salesByLabel = {};
    allUnits.forEach((unit) => {
      const type = unit.unit_type || "Unknown";
      if (type !== "Unknown") {
        salesByLabel[type] =
          (salesByLabel[type] || 0) + (parseFloat(unit.sales_value) || 0);
      }
    });
  ...
  const deliveryComplianceChartData = useMemo(() => {
    ...
    units.forEach((unit) => {
      ...
      try {
        ...
        const adjusted = new Date(contractDate);
        adjusted.setMonth(
          adjusted.getMonth() + (parseInt(unit.grace_period_months) || 0),
        );
    ...
    <>
      ...
      >
        <div className="chart-card">
          <div className="chart-header">
            ...
            <button
              className="chart-btn chart-expand-btn"
              onClick={() =>
                openChartModal("Inventory Status", inventoryChartData, "pie")
              }
              title="Expand chart"
            >
              ⛶
        ...
        <div className="chart-card">
          ...
          <div style={{ height: "250px" }}>
            <Bar
              id="payment-plan-chart"
              data={paymentPlanChartData}
              options={barOptions}
            />
          </div>
        </div>
        ...
        <div className="chart-card">
          <div className="chart-header">
            ...
            <button
              className="chart-btn chart-expand-btn"
              onClick={() =>
                openChartModal("Sales Trend", salesTrendChartData, "line")
              }
              title="Expand chart"
            >
              ⛶
          ...
          <div className="chart-controls-wrapper">
            <div className="date-range-controls compact">
              ...
              <input
                ...
                value={salesDateRange.end}
                onChange={(e) =>
                  setSalesDateRange({ ...salesDateRange, end: e.target.value })
                }
              />
              <button
                className="btn btn-primary btn-sm"
                onClick={() => {
                  const start = salesDateRange.start
                    ? new Date(salesDateRange.start)
                    : null;
                  const end = salesDateRange.end
                    ? new Date(salesDateRange.end)
                    : null;
                  onDateRangeChange("salesDateRange", start, end);
                }}
              >
            ...
            <div className="delivery-section">
              ...
              <div className="delivery-analysis" style={{ minWidth: 0 }}>
                ...
                <div className="chart-scroll-wrapper">
                  <div style={{ height: "220px", minWidth: "600px" }}>
                    <Line
                      data={deliveryProgressChartData}
                      options={lineOptions}
                    />
                  </div>
                </div>
                ...
                <div className="chart-controls-wrapper">
                  <div className="date-range-controls compact">
                    ...
                    <button
                      className="btn btn-primary btn-sm"
                      onClick={() => {
                        const start = deliveryDateRange.start
                          ? new Date(deliveryDateRange.start)
                          : null;
                        const end = deliveryDateRange.end
                          ? new Date(deliveryDateRange.end)
                          : null;
                        onDateRangeChange("deliveryDateRange", start, end);
                      }}
                    >
              ...
              <div className="delivery-compliance" style={{ minWidth: 0 }}>
                ...
                >
                  <Pie
                    data={deliveryComplianceChartData}
                    options={deliveryCompliancePieOptions}
                  />
                </div>
              </div>
            </div>
      ...
      >
        {chartModalData && (
          ...
          >
            {chartModalType === "pie" && (
              <Pie data={chartModalData} options={modalPieOptions} />
            )}
            {chartModalType === "bar" && (
              <Bar data={chartModalData} options={barOptions} />
            )}
            {chartModalType === "line" && (
              <Line data={chartModalData} options={lineOptions} />
            )}
          </div>
        )}
      </ChartModal>
