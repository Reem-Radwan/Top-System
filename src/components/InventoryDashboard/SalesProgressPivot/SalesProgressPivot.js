...
const SalesProgressPivot = ({ units }) => {
  ...
  // Change toggleCity and toggleProject functions:
...
const toggleProject = (cityName, projectName) => {
 ...
 // In toggleExpandCollapse function, update the expand logic:
const toggleExpandCollapse = () => {
  const hasExpanded = Object.values(expandedCities).some(v => v === true) || 
                      Object.values(expandedProjects).some(v => v === true);
  ...
  if (hasExpanded) {
    ...
    // Expand all - initialize all to true
    const newCities = {};
    const newProjects = {};
    progressData.cityData.forEach(city => {
      newCities[city.name] = true; // Set explicitly to true
      Object.keys(city.projects).forEach(projectName => {
        newProjects[`${city.name}-${projectName}`] = true; // Set explicitly to true
      });
    });
    setExpandedCities(newCities);
    setExpandedProjects(newProjects);
    ...
    <div className="sales-progress-container">
      ...
      {!comparisonMode && !yearsAnalysisMode && (
        <div className="year-tabs">
          ...
          {/* <button className='reem'>Reem</button> */}
        </div>
      )}
